...
int get_device_info(int i)
{
	...
	if (!c.sparse_mode) {
		...
		if (S_ISBLK(stat_buf->st_mode) &&
				!c.force && c.func != DUMP && !c.dry_run) { //get in here_mj!
	...
	if (c.sparse_mode) {
...
#ifdef BLKSSZGET
		MSG(0, "MINJEE_2\n");
		mj_num = ioctl(fd, BLKSSZGET, &sector_size);
		MSG(0, "sector_size: %u\n", sector_size);
		MSG(0, "MINJEE_3\n");
		...
		if (mj_num < 0)
			...
			MSG(0, "dev->sector_size/ sector_size: %u/%u\n", dev->sector_size, sector_size);
			dev->sector_size = sector_size;
		}
#endif
#ifdef BLKGETSIZE64
		MSG(0, "MINJEE_4\n");
		mj_num = ioctl(fd, BLKGETSIZE64, &dev->total_sectors);
		MSG(0, "dev-> total_secotrs : %llu\n", dev->total_sectors);
		MSG(0, "MINJEE_5\n");
...
#else
		MSG(0, "MINJEE_5\n");
		mj_num = ioctl(fd, BLKGETSIZE, &total_sectors);
		MSG(0, "&total_sectors : %u\n", total_sectors);
		MSG(0, "MINJEE_6\n");
		if (mj_num < 0) {
			MSG(0, "\tError: Cannot get the device size\n");
			free(stat_buf);
		...
		MSG(0, "MINJEE_7\n");
#endif
		if (i == 0){ //when it is cns part
			dev->total_sectors /= dev->sector_size;
			MSG(0, "/dev/nvme1n1(cns)dev->total_sectors/ dev->sector_size : %llu, %u\n", dev->total_sectors,dev->sector_size);
		}
		else{ //when it is zns part
			dev->total_sectors = 29360128;
		}
